...
class CalculatorApp : Window
{
    ...
    private List<string> history; // To store the calculation history

    public CalculatorApp() : base("Simple Calculator")
    {
        history = new List<string>(); // Initialize the history list
        ...
        Table table = new Table(5, 4, true); // Increased rows to 5 for the history button

        string[] labels = { "7", "8", "9", "/",
                            "4", "5", "6", "*",
                            "1", "2", "3", "-",
                            "0", "C", "=", "+",
                            "History" }; // Added History button
        ...
        else
        {
            switch (label)
            {
                ...
                case "=":
                    ...
                    history.Add($"{display.Text} = {calculationResult}"); // Store the result in history
                    display.Text = calculationResult;
                    newInput = true;
                    lastOperator = "";
                    break;
                case "History":
                    ShowHistory(); // Call the method to show history
                    break;
                default:
                    if (!newInput)
                    {
                        ...
                        history.Add(currentExpression + $" = {result}"); // Store the operation in history
        ...
        switch (lastOperator)
        {
            ...
            case "/":
                if (input != 0)
                    ...
                    display.Text = "Error"; // Handle division by zero
                break;
            default:
                result = input;
                break;
